...
function molecularWeightCalculator(formula)
{
    ...
    getSymbolMap();
    //Find the weights of each symbol in map and store them
    symbolMap.forEach(storeWeight); //search through symbol map and find the weight assoicate from the periodic table
    return calculateWeight();


    /*inner functions*/
    function storeWeight(value, key, map)
    {
        //searches for match
        periodicTable.elements.some(function(element,index,array)
        {
            if(element.symbol === key) //when the mathcing element is found
            {
                //add its weigh to the weight map
                weightMap.set(key, element.atomic_mass);
                return true;//returning true breaks the loop
            }
        });
    }
            ...
            for(var i = 0; i< formula.length; i++)
            {
                if(formula.charAt(i).search(capitalLetter)!= -1)//if the character is a capital letter
                {
                    ...
                    else if(formula.charAt(i+1).search(lowerCaseLetter) != -1)//if the char after a capital letter is a lowercase
                    {
                        ...
                        if(i+1 === formula.length-1)// if the lowercase letter found is the last char in string
                        {
                            onFindSymbol();
                        }
                        else if(formula.charAt(i+2).search(digit) != -1)//if next symbol is a number
                        {
